...
//
// V28 NCRISC — PER-BATCH-FOOTPRINT (mirror of v35_gather_brisc_pbf.cpp for fy). Same
// per-batch footprint pick: a 1024-cell batch fully inside the small region loops (K0,H0),
// else (max_k,max_h). base_idx still comes from BRISC via CB_BASE; drain unchanged. The
// SFPU (v28_compute_pbf) consumes the same per-batch (mk,mh) tile count → all three agree.
...
void kernel_main() {
    ...
    const uint32_t n_small    = get_arg_val<uint32_t>(13);
    const uint32_t K0         = get_arg_val<uint32_t>(14);
    const uint32_t H0         = get_arg_val<uint32_t>(15);
    ...
    for (uint32_t b=0;b<n_batches;++b) {
        ...
        for (uint32_t k=0;k<mk;++k) for (uint32_t h=0;h<mh;++h) {
            { DeviceZoneScopedN("NB-FYRESV"); cb_reserve_back(CB_FY,1); }
            { DeviceZoneScopedN("NB-FYFILL");
        ...
        { DeviceZoneScopedN("NB-DRAINWAIT"); cb_wait_front(GX,1); cb_wait_front(GY,1); }
