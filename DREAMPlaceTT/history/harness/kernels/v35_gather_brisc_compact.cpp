    ...
    for (uint32_t b = 0; b < n_batches; ++b) {
        ...
        for (uint32_t k=0;k<max_k;++k) {
            for (uint32_t h=0;h<max_h;++h) {
                { DeviceZoneScopedN("GB-FXRESV"); cb_reserve_back(CB_FX,1); }  // BRISC blocked by SFPU (CB full) if large
                { DeviceZoneScopedN("GB-FXFILL");                              // the actual fx-tile gather from L1 band
