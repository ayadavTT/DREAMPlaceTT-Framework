...
//
// V18 NCRISC Scatter Kernel (RISCV_1, NOC_1).
//
// V18 = V11 + per-source hash-table pre-aggregation. Each (bx, by, area)
// tuple is summed into a per-RISC L1 hash table keyed by (bx<<16)|by during
// the per-tile loop. At end-of-batch we walk the table and emit one tuple
// per unique (bx, by) to the per-owner staging buffers, then the existing
// flush_recv path delivers them to route_buf.
//
// Wire format on route_buf is identical to V11 — receiver gather kernels
// see fewer tuples per page (mega-tile bin gets ~1 per source instead of N)
// but otherwise no protocol change.
//
// Two extra runtime args vs V11:
//  23: v18_hash_off        L1 byte offset of hash table region (keys then areas)
//  24: v18_hash_bits       log2(num_slots); slots = 1 << bits
//
...
// layout in v11_scatter_b_dm.cpp / v18_scatter_b_dm.cpp).
...
// 5-instruction integer hash mixer. Avoids 64-bit multiplies (expensive on
// BRISC) while spreading sequential (bx<<16|by) keys across the table.
...
void kernel_main() {
    ...
    // V18 hash table + dirty list (slot indices of inserted keys).
    ...
    // V18 hash table. Layout: keys[hash_slots] (uint32) then areas[hash_slots]
    // (float in fp32 mode, uint16 bf16 bits in bf16 mode). Dirty list is a
    // uint16[hash_slots] kept after areas when not in no-dirty mode.
    ...
    // ── V18: clear hash table (empty sentinel = 0xFFFFFFFF) ───────────────
    // keys[] only; areas[] is written on first insert so its prior content
    // doesn't matter.
    {
    ...
    // Mid-batch hash-flush threshold: leave 256-slot headroom so even a
    // pathological probe chain after the flush still finds an empty slot.
    // Bigblue1_2048 has 1.7× more cells than adaptec1_2048; per-RISC unique-bin
    // count can exceed 32K slots, which without this guard caused an infinite
    // probe loop on the first scatter call.
    ...
    // Drain the hash table into staging and clear keys. Walks either the
    // dirty list (when v18_no_dirty=0) or the full table (no_dirty=1).
    ...
    for (uint32_t t = 0; t < n_tiles; ++t) {
        ...
        *dr_sem = t + 1u;
        ...
        // Drain mid-batch if the hash table is nearly full. Prevents the
        // infinite probe loop that fired on bigblue1_2048 (per-RISC unique-bin
        // count > hash_slots).
        ...
        }
        {
            ...
            // pre-dispatch. Owner is computed at flush time from tile_to_core.
            // Per (j,k) we just hash-insert (bx,by) → accumulate area.
                            ...
                            for (; probes < 64u; ++probes) {
                                ...
                                if (k_at == V18_KEY_EMPTY) {
                                    ...
                                    if (!v18_no_dirty) {
                                        ...
                                        v18_dirty_count++;  // for threshold tracking
                                    }
                                    inserted = true; break;
                                }
    ...
    // ── V18 Step 2a: drain remaining hash entries into staging ────────────
    {
        DeviceZoneScopedN("V18N-HASH-FLUSH");
        v18_drain_hash();
    }

    // ── Step 2b: final flush of partial buffers ───────────────────────────
    {
