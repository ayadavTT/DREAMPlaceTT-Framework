#!/usr/bin/env bash
# Isolated test + profile for the v11-scatter kernel. Self-contained: uses only
...
for DES in $DESIGNS; do
  for G in $GRIDS; do
    echo "================ $DES grid $G ================"
    "$PY" -u "$HERE/v11_scatter_test.py" --grid "$G" --design "$DES" $QUICK || echo "[run.sh] $DES grid $G FAILED"
  done
done
