...
_KDIR = os.path.dirname(_HERE)                 # .../v19-scatter
...
os.environ.setdefault("GATHER_MODE", "v19")
...
def run(grid: int, design: str, n_iters: int = 4):
    ...
    print(f"[v19-test] building engine grid={grid} design={design} NC_max={nc} ...", flush=True)
    eng = v19_engine.V19Engine(grid, grid, nc, 0.0, 0.0, float(grid), float(grid), "v19")
    if True:
        for dist in synth.DISTS:
            ...
            print(f"[v19-test] {design:9s} grid={grid:5d} {dist:8s} nc={nc:8d} "
...
def write_reports(rows, quick):
    ...
    for r in rows:
        merged[(r["design"], str(r["grid"]), r["dist"])] = dict(
            kernel="v19-scatter", design=r["design"], grid=r["grid"], dist=r["dist"],
    ...
    with open(md, "w") as f:
        f.write(f"# v19-scatter — measured report{' (quick)' if quick else ''}\n\n")
    ...
    print(f"[v19-test] wrote {csvp}\n[v19-test] wrote {md}  ({npass}/{len(rows)} pass)", flush=True)
