...
//
// V12 NCRISC Combined Kernel (RISCV_1, NOC_1) — Phase 1 Scatter + Phase 2 Density.
//
// Single kernel_main() running both phases sequentially:
//
//   Phase 1 (Scatter):
//     Reads cell data (second half of each SoA tile) from DRAM, computes FP32
//     overlaps, routes V12Overlap vectors to tile-owning cores via route_buf.
//
//   Barrier:
//     Increments coordinator's barrier semaphore, then waits for
//     phase2_go_sem_n to be set by coordinator BRISC.
//
//   Phase 2 (Density writeback):
//     Drains CB_ACCUM (FP32 tiles from TRISC), applies inv_bin_area scaling,
//     and writes 32 row-slices per tile to density_buf in DRAM.
//
// Runtime args (args 0..26 + per-tile indices):
//   0..23:  Same scatter args as v12_brisc_combined.cpp.
//   22:     phase2_go_sem_n_id  (NCRISC waits on this, not phase2_go_sem_b)
//   23:     inv_ba_u32          (inv_bin_area as bitcast uint32)
//   24:     n_tiles_owned
//   25:     density_dram        DRAM base of density_buf
//   26:     density_pgsz        page size of density_buf
//   27 .. 27+n_tiles_owned-1:  tile linear indices
...
static constexpr uint32_t NCRISC_STAGE_CAP = 4u;
...
void kernel_main() {
    ...
    const uint32_t phase2_go_sem_n   = get_arg_val<uint32_t>(22);  // NCRISC go
    const uint32_t inv_ba_u32        = get_arg_val<uint32_t>(23);
    const uint32_t n_tiles_owned     = get_arg_val<uint32_t>(24);
    const uint32_t density_dram      = get_arg_val<uint32_t>(25);
    const uint32_t density_pgsz      = get_arg_val<uint32_t>(26);

    union { uint32_t u; float f; } cv; cv.u = inv_ba_u32;
    const float inv_bin_area = cv.f;

    // ── L1 layout: mirror BRISC layout up to NCRISC scatter region ───────────
    uint8_t* base = reinterpret_cast<uint8_t*>(get_write_ptr(CB_SCRATCH));
    uint32_t off  = 0u;

    // tile_to_core (BRISC loads; NCRISC reads the same L1 region after barrier).
    uint16_t* tile_to_core = reinterpret_cast<uint16_t*>(base + off);
    off += tile_map_bytes;
    off  = (off + 63u) & ~63u;

    // NOC table (just after tile_to_core in DRAM and L1).
    const uint32_t noc_table_bytes = nc_all * 2u * sizeof(uint32_t);
    off += noc_table_bytes;
    off  = (off + 63u) & ~63u;

    // Skip BRISC cell buffers.
    off += 4u * TILE_BYTES;
    off  = (off + 63u) & ~63u;

    // Skip BRISC staging region.
    constexpr uint32_t BRISC_STAGE_CAP = 4u;
    off += nc_all * BRISC_STAGE_CAP * (uint32_t)sizeof(V12Overlap); off = (off + 3u) & ~3u;
    off += nc_all * sizeof(uint32_t);  // brisc staging_count
    off += nc_all * sizeof(uint32_t);  // brisc dram_off_vecs
    off  = (off + 63u) & ~63u;
    off += V12_HDR_BYTES;              // brisc hdr_scratch
    off  = (off + 63u) & ~63u;

    // NCRISC staging.
    V12Overlap* staging      = reinterpret_cast<V12Overlap*>(base + off);
    off += nc_all * NCRISC_STAGE_CAP * (uint32_t)sizeof(V12Overlap); off = (off + 3u) & ~3u;
    ...
    // Skip BRISC Phase 2 accum region to get to NCRISC density tile buffer.
    // BRISC Phase 2 allocates: inbound_hdrs, ov_stage, ox_scratch, oy_scratch.
    off += nc_all * V12_HDR_BYTES;  off = (off + 63u) & ~63u;
    off += 8u * 80u;                off = (off + 63u) & ~63u;
    off += 2u * (TILE_W * TILE_H * sizeof(uint16_t)); off = (off + 63u) & ~63u;

    // NCRISC density tile buffer.
    float* density_tile = reinterpret_cast<float*>(base + off);
    ...
    // NCRISC writes to the FIRST HALF of each route page (right after header).
    ...
    // Load tile_to_core (same DRAM page as BRISC load — NCRISC reads on NOC_1).
    {
    ...
    // ── PHASE 1: Scatter (NCRISC — second half of each SoA tile) ─────────────
    {
            ...
            for (uint32_t ci = TILE_ELEMS / 2u; ci < TILE_ELEMS; ++ci) {
                ...
                for (uint32_t tx = tx_lo; tx <= tx_hi; ++tx) {
                    for (uint32_t ty = ty_lo; ty <= ty_hi; ++ty) {
                        ...
                        if (staging_count[owner] >= NCRISC_STAGE_CAP) flush_recv(owner);
                    }
                }
            }
        }
    ...
    // Write NCRISC count to the NCRISC header region (bytes 0..31 of the page).
    // BRISC concurrently writes bytes 32..63 (its own region), so both writes
    // are to non-overlapping 32-byte aligned regions — no race condition.
    {
        ...
        for (uint32_t r = 0u; r < nc_all; ++r) {
            ...
            // Write 32 bytes to byte-offset 0 (NCRISC region; naturally aligned).
    ...
    // ── BARRIER: increment coordinator, wait for go ───────────────────────────
    {
        ...
        volatile tt_l1_ptr uint32_t* go_sem =
            reinterpret_cast<volatile tt_l1_ptr uint32_t*>(get_semaphore(phase2_go_sem_n));
    ...
    for (uint32_t tile_local = 0u; tile_local < n_tiles_owned; ++tile_local) {
        ...
        uint32_t tile_idx = get_arg_val<uint32_t>(27u + tile_local);
        uint32_t tx = tile_idx / N_tiles;
        uint32_t ty = tile_idx % N_tiles;

        // Wait for BRISC to push the FP32 tile to CB_ACCUM (TRISC is no-op).
        cb_wait_front(CB_ACCUM, 1u);

        // Scale + copy to local scratch.
        {
        ...
        // Write 32 bx rows to density_buf.
        {
            ...
            for (uint32_t bxw = 0u; bxw < TILE_W; ++bxw) {
                ...
                uint64_t dst_addr  = page_base + (uint64_t)(ty * TILE_H) * sizeof(float);
                uint32_t src_l1    = reinterpret_cast<uint32_t>(&density_tile[bxw * TILE_W]);
