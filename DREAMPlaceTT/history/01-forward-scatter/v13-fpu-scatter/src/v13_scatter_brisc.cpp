...
void kernel_main() {
    ...
    auto flush_recv = [&](uint32_t recv, uint32_t cnt) {
        ...
        for (uint32_t i = to_write; i < cnt_padded; ++i) {
            ...
            // tile_id=0xFFFF marks padding records explicitly so V15's bucket
            // lookup naturally skips them (owned_lookup[r.tile_id] is bounds-
            // checked separately; we compare against 0xFFFF first). V13 gather
            // already skipped them via `r.tile_id != target_tile_id` since no
            // valid tile_id ever equals 0xFFFF.
            pad_rec.tile_id = 0xFFFFu; pad_rec.pad = 0;
