...
//
// V35 COMPACT-RECORD microbench — identical to v35live_host.cpp except the grouped
// buffer uses 64 B (16 u32) records instead of 128 B (32 u32). place transcodes the
// 128 B source stash -> 64 B (drops kc/hc, trims px/py to max_k/max_h); gather reads
// 64 B. Goal: measure the place + gather speedup from halving the grouped-buffer
// write/read, validated bit-exact vs the CPU reference (rel_l2). Source stash stays
// 128 B (matches the production forward), so this models a BACKWARD-ONLY change.
//   CLI: v35compact [N M ncells max_k max_h cluster]   (requires 4+max_k+max_h <= 16)
...
int main(int argc,char**argv){
    ...
    if(4u+max_k+max_h>16u){ printf("[v35compact] ERROR: 4+max_k+max_h=%u > 16 (record won't fit 64 B)\n",4u+max_k+max_h); return 1; }
    ...
    // ── source stash: 128 B/cell (32 u32), SAME as the production forward V31_GEOM ──
    ...
    const uint32_t REC_C=16u;                                            // 64 B compact record
    const uint32_t grouped_pg=(uint32_t)((uint64_t)ncells*64u + 64u);    // 64 B records (64-B aligned)
    ...
    // ── PASS 1: count ── (unchanged — reads 128 B source stash)
    ...
    // ── PASS 2: place (COMPACT) ── CB_SORT holds 64 B records; place gets max_k/max_h
    Program pp=CreateProgram();
    cbf(pp,0,pchunk*128u,1); cbf(pp,1,pchunk*64u,1); cbf(pp,2,tb_pg,1); cbf(pp,3,plan_pg,1); cbf(pp,4,((4u*ntiles*4u+63u)&~63u),1);
    auto kp=CreateKernel(pp,std::string(DENSITY_KERNEL_DIR)+"v35_place_compact.cpp",all_crs,DataMovementConfig{.processor=DataMovementProcessor::RISCV_0,.noc=NOC::RISCV_0_default});
    for(int c=0;c<nc;++c){uint32_t f=(uint32_t)c*cpc_cell,n=std::min(cpc_cell,(f<ncells)?(ncells-f):0u);
        SetRuntimeArgs(pp,kp,ccs[c],{(uint32_t)c,f,n,sta,stash_pg,ntiles,W,gra,grouped_pg,tba,spa,plan_pg,pchunk,max_k,max_h});}
    ...
    // ── DEBUG: verify compact grouped records (64 B layout) ──
    if(getenv("V35_VERIFY_PLACE")){
        ...
        for(uint32_t p=0;p<ncells;++p){ uint32_t oi=g2o[p]; if(oi>=ncells)continue;
            const uint32_t* o=&gb[(size_t)p*REC_C]; const Cell&cl=cells[oi];   // 64 B compact, bxl global
            bool bad = (o[0]!=oi)||(o[1]!=cl.bxl)||(o[2]!=cl.byl)||(fb(o[3])!=cl.ratio);
            for(uint32_t k=0;k<max_k;++k){ if(fb(o[4u+k])!=cl.px[k])bad=true; }
            for(uint32_t h=0;h<max_h;++h){ if(fb(o[4u+max_k+h])!=cl.py[h])bad=true; }
            if(bad){ ++pbad; if(nshow<6){++nshow; printf("[place_bad] p=%u oi=%u bxl(%u/%u) ratio(%.3f/%.3f) px0(%.3f/%.3f)\n",
                p,oi,o[1],cl.bxl,fb(o[3]),cl.ratio,fb(o[4]),cl.px[0]); } } }
        printf("[place_verify] compact full scan: %zu / %u records mismatch %s\n", pbad,ncells,pbad?"<<< PLACE BUG":"(place OK)");
    }

    // ── PASS 3: gather (COMPACT brisc + v28 ncrisc + v28 compute) ──
    ...
    const uint32_t fld_bytes=band_cols*M*4u+(max_h+8u)*4u, chunk_batches=4, chunk_bytes=chunk_batches*1024u*64u;  // 64 B records
    ...
    // ── validate grad vs CPU multi-bin (grouped order via g2o) — format-independent ──
    ...
    printf("[v35compact] N=%u M=%u ncells=%u nc=%d cluster=%u W=%u ntiles=%u maxn=%u imbal=%.2fx checked=%zu REC=64B\n",N,M,ncells,nc,cluster,W,ntiles,maxn,imbal,checked);
    printf("[v35compact] count=%.3f plan=%.3f place=%.3f gather=%.3f | PREP=%.3f TOTAL=%.3f ms\n",count_ms,plan_ms,place_ms,gather_ms,prep_ms,total_ms);
    printf("[v35compact] accuracy: maxabs=%.6g rel_l2=%.6g nbad=%zu %s\n",maxabs,rel_l2,nbad,(rel_l2<1e-4&&nbad==0)?"OK":"CHECK");
    printf("V35COMPACT_RESULT,%u,%u,%u,%u,%u,%u,%.3f,%.3f,%.3f,%.3f,%.6g\n",N,M,ncells,max_k,max_h,cluster,count_ms,place_ms,gather_ms,total_ms,rel_l2);
    return 0;
}
