...
void kernel_main() {
    ...
    for (uint32_t t = 0u; t < n_owned_tiles; ++t) {
...

        {
            ...
            while (true) {
                {
                    DeviceZoneScopedN("V13C-WAIT-OX");
                ...
                {
                    DeviceZoneScopedN("V13C-WAIT-OY");
                    cb_wait_front(CB_OY, N_INFLIGHT);
                }

                // UNPACK side: one HW-batched paired-unpack MOP dispatch loads
                // all N_INFLIGHT (SrcA, SrcB) tile pairs back-to-back without
                // per-call STALLWAIT/WRCFG overhead. Replaces 4× separate
                // llk_unpack_AB_matmul calls (~24 µs each).
                {
                    DeviceZoneScopedN("V13C-UNPACK");
