...
void kernel_main() {
    ...
    { DeviceZoneScopedN("V35-PLACE");
      ...
      while (done < my_n) {
          ...
          // transcode 128 B source -> 64 B compact, into the tile-sorted run
          for (uint32_t i=0;i<this_n;++i){ uint32_t t=rec[i*REC_S+1u]/W; if(t>=ntiles)t=ntiles-1u;
              ...
              sorted[d+0]=s[0]; sorted[d+1]=s[1]; sorted[d+2]=s[2]; sorted[d+3]=s[5];   // gidx,bxl,byl,ratio
              for (uint32_t k=0;k<max_k;++k) sorted[d+4u+k]=s[6u+k];                    // px[0..max_k)
              for (uint32_t h=0;h<max_h;++h) sorted[d+4u+max_k+h]=s[14u+h];             // py[0..max_h)
              cur[t]++; }
