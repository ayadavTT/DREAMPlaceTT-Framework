...
void kernel_main() {
    ...
    for (uint32_t t = 0; t <= n_tiles; ++t) {
        ...
        // Stage B': outer products from c_22..c_29. NCRISC publishes the BASE
        // address of each c_(22+J) via the ox_ptr slot (re-purposed; oy_ptr
        // is unused in V11-outer mode). Each base holds 8 tiles (K=0..7) at
        // OUTER_TILE_BYTES = 1024 floats × 4 = 4 KB stride.
        constexpr uint32_t OUTER_TILE_BYTES = TILE_ELEMS * sizeof(float);  // 4096
        const float* outer_data[V11_MAX_OVERLAP][V11_MAX_OVERLAP];
        for (uint32_t j = 0; j < V11_MAX_OVERLAP; ++j) {
            uint32_t base = shared->ox_ptr[j];
            for (uint32_t k = 0; k < V11_MAX_OVERLAP; ++k) {
                outer_data[j][k] = reinterpret_cast<const float*>(base + k * OUTER_TILE_BYTES);
            }
        }

        // V18: mid-batch drain if hash near capacity.
...

        {
            ...
            for (uint32_t ci = TILE_ELEMS / 2u; ci < TILE_ELEMS; ++ci) {
                ...
                // V18-outer: area from precomputed SFPU outer-product. V18:
                // hash-agg (bx, by, area). No shard table / emit_counter.
                for (uint32_t j = 0; j < V11_MAX_OVERLAP; ++j) {
    ...
    // ── V18: drain remaining hash entries ─────────────────────────────────
    {
