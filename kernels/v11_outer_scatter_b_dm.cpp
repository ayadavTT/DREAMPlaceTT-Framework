    ...
    for (uint32_t t = 0; t <= n_tiles; ++t) {
...

        {
            ...
            for (uint32_t ci = TILE_ELEMS / 2u; ci < TILE_ELEMS; ++ci) {
                ...
                // Stage B': no j-loop early-exit (we don't have ox_j directly;
                // the inner k-loop early-exits on area <= 0 which covers both
                // ox_j == 0 and oy_k == 0).
                for (uint32_t j = 0; j < V11_MAX_OVERLAP; ++j) {
                    ...
                    for (uint32_t k = 0; k < V11_MAX_OVERLAP; ++k) {
                        // Stage B': area is precomputed by SFPU; just read it.
